                    ...
                    if let ratatui::crossterm::event::KeyCode::Char(c) = event.code {
                        ...
                        if has_mapping {
                            ...
                            if let Err(e) = func.call::<_, ()>(()) {
                                self.status = format!("Lua execution error: {}", e);
                            }

                            self.key_sequence.clear();
                        } else if self.key_sequence.len() > 3 {
                            // Arbitrary timeout/clear logic
                            self.key_sequence.clear();
                            self.status = format!("Invalid sequence: {}", sequence);
                        }

                        // 3. Drain and execute the Command Queue generated by Lua
